import pickle
import streamlit as st
import requests

def fetch_poster(id):
    url= "https://api.themoviedb.org/3/movie/{}?api_key=8265bd1679663a7ea12ac168da84d2e8&language=en-US".format(id)
    data = requests.get(url)
    data = data.json()
    poster_path = data['poster_path']
    full_path = "http://image.tmdb.org/t/p/w500/" + poster_path
    return full_path

def recommend(movie):
    index = movies[movies['title'] == movie].index[0]
    distances = sorted(list(enumerate(similarity[index])), reverse=True, key = lambda x: x[1])
    recom = []
    recom_poster = []
    for i in distances[1:6]:
        id = movies.iloc[i[0]].id
        recom_poster.append(fetch_poster(id))
        recom.append(movies.iloc[i[0]].title)
    return recom, recom_poster

st.header("Content Recommendation System")
movies = pickle.load(open('../code/pkls/movie_list.pkl', 'rb'))
similarity = pickle.load(open('../code/pkls/similarity.pkl', 'rb'))

movie_list = movies['title'].values
selected_movie = st.selectbox(
    'Select content for recommendation',
    movie_list
)

if st.button('Show'):
    recom, recom_poster = recommend(selected_movie)
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.text(recom[0])
        st.image(recom_poster[0])
    
    with col2:
        st.text(recom[1])
        st.image(recom_poster[1])

    with col3:
        st.text(recom[2])
        st.image(recom_poster[2])
    
    with col4:
        st.text(recom[3])
        st.image(recom_poster[3])

    with col5:
        st.text(recom[4])
        st.image(recom_poster[4])